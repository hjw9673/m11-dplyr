...
setwd("~/Desktop/INFO201/m11-dplyr/exercise-6")
...
avg.arrival.delay <- flights %>% group_by(dest) %>% summarise(avg.delay = mean(arr_delay, na.rm = TRUE)) %>% 
  mutate(faa = dest) %>% left_join(airports, by = 'faa') %>% arrange(-avg.delay)
...
avg.delay.airline <- flights %>% group_by(airlines) %>% summarise(avg.delay = mean(arr_delay, na.rm = TRUE)) %>%
  left_join(airlines, by = 'carrier') %>% arrage(-avg.delay)
